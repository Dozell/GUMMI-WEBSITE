...
/*
 * TO GO LIVE:
 * Remove `soon: true` from the relevant card and replace href: '#' with the real URL.
 */
const socials = [
  {
    name: 'X / Twitter', handle: '@GummiCoin',      icon: '𝕏',  desc: 'Memes, updates, and bear energy.',
    border: 'hover:border-white/30',    bg: 'hover:bg-white/5',     href: '#', soon: true,
  },
  {
    name: 'Telegram',    handle: 't.me/GummiCoin',   icon: '✈️', desc: 'The main community chat.',
    border: 'hover:border-sky-400/30',  bg: 'hover:bg-sky-500/5',   href: '#', soon: true,
  },
  {
    name: 'Pump.fun',    handle: 'Buy $GUMMI',        icon: '🚀', desc: 'Official launch page.',
    border: 'hover:border-green-400/30',bg: 'hover:bg-green-400/5', href: '#', soon: true,
  },
  {
    name: 'DexScreener', handle: 'Track $GUMMI',      icon: '📊', desc: 'Watch the chart go.',
    border: 'hover:border-green-400/30',bg: 'hover:bg-green-400/5', href: '#', soon: true,
  },
];
...
export default function Community() {
  ...
  return (
    <section id="community" className="relative py-20 sm:py-28 bg-[#050a05] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6" ref={ref}>
        ...
        >
          ...
          <p className="text-white/50 text-base sm:text-lg max-w-xl mx-auto">
            ...
            Links are coming — the bears are still setting things up.
          </p>
        </motion.div>
            ...
            >
              {s.soon ? (
                <div className={`relative bg-white/4 border border-white/10 rounded-2xl p-6 text-center transition-all ${s.border} ${s.bg} cursor-not-allowed select-none`}>
                  <span className="absolute top-3 right-3 bg-yellow-400/15 text-yellow-300 text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-wide">
                    Soon
                  </span>
        ...
        >
          <div className="text-center sm:text-left">
            ...
            <p className="text-white/45 text-sm">
              The sweetest community is forming. Get in before the bear starts dancing.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 shrink-0 w-full sm:w-auto">
            {/* ← Replace with real X link when ready */}
            <button
              disabled
              className="flex items-center justify-center gap-2 bg-white/8 border border-white/15 text-white/45 font-bold px-5 py-3 rounded-xl text-sm cursor-not-allowed w-full sm:w-auto"
            >
              𝕏 Follow on X — Coming Soon
            </button>
            <button
              disabled
              className="flex items-center justify-center gap-2 bg-sky-500/8 border border-sky-500/15 text-sky-400/45 font-bold px-5 py-3 rounded-xl text-sm cursor-not-allowed w-full sm:w-auto"
            >
